...
if (!file.exists("chronicle.rda")) 
    download.file("http://capa.ddb.umu.se/~gb/IDS/chronicle.rda", "chronicle.rda")
if (!file.exists("starting.rda")) 
    download.file("http://capa.ddb.umu.se/~gb/IDS/starting.rda", "starting.rda")
...
Then we introduce two help variables, **lopnr** and **antrec**. They will help us keeping track of where in an individual's records we presently are, especially if we are at the first or last record. The function **rc** in the package **skel14* help us with that.
```{r recnum}
library(skel14)
...
```{r deadplot, echo = FALSE, cache = TRUE}
...
fit <- coxph(Surv(enter, exit, Type == "childBirth") ~ civst + birthdate, data = episodes[episodes$sex == "woman" & episodes$childBirth == 1, ])
summary(fit)
plot(survfit(fit), mark.time = FALSE, xlab = "Age")
```
...
mark.time = FALSE, xlim = c(0, 10), xlab = "Years", ylab = "Cumulative hazards", main = "First birth interval")
